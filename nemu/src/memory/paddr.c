...
static uint8_t pmem[CONFIG_MSIZE + 8] PG_ALIGN = {};
#endif
...
void init_mem() {
  ...
  int i;
  for (i = 0; i < (int)(CONFIG_MSIZE / sizeof(p[0])); i++) { p[i] = rand(); }
#endif
...
#ifdef CONFIG_MTRACE
  // if (addr >= CONFIG_MTRACE_START && addr <= CONFIG_MTRACE_END + CONFIG_MT
#endif
