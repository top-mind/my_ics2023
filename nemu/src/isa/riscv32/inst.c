...
#define INSTPAT_MATCH(s, ...) __VA_ARGS__
  INSTPAT_START(ras);
  INSTPAT("??????? ????? ????? ??? 00?01 11011 11", ras_push());
  // rd=x1/x5
  // XXX func3 of jalr
  INSTPAT("??????? ????? 00101 000 00001 11001 11", ras_pop(), ras_push());
  INSTPAT("??????? ????? 00001 000 00101 11001 11", ras_pop(), ras_push());
  INSTPAT("??????? ????? ????? 000 00?01 11001 11", ras_push());
  INSTPAT("??????? ????? 00?01 000 ????? 11001 11", ras_pop());
  INSTPAT_END(ras);
}
#endif
