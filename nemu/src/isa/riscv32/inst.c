...
#define INSTBRANCH                                            \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr, I, R(rd) = s->snpc, s->dnpc = (src1 + imm) >> 1 << 1);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", beq, B, if ((sword_t)src1 < (sword_t)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", beq, B, if ((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", beq, B, if (src1 < src2) s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", beq, B, if (src1 >= src2) s->dnpc = s->pc + imm);

  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb, I, R(rd) = SEXT(Mr(src1 + imm, 1), 4));
