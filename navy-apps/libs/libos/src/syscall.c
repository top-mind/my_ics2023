...
intptr_t program_break = 0x00000000;
...
int _read(int fd, void *buf, size_t count) {
  return SYSTEM_CALL(SYS_read, fd, (intptr_t)buf, count);
}
...
int _gettimeofday(struct timeval *tv, struct timezone *tz) {
  return SYSTEM_CALL(SYS_gettimeofday, (intptr_t)tv, (intptr_t)tz, 0);
}

int _execve(const char *fname, char * const argv[], char *const envp[]) {
  return SYSTEM_CALL(SYS_execve, (intptr_t)fname, (intptr_t)argv, (intptr_t)envp);
}
